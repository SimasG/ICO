...
const Home: NextPage = () => {
    ...
    const getTokensToBeClaimed = async () => {
        try {
            ...
            // ** Why would I need a signer here? Can't I get the MetaMask address w provider?
            const signer = await getProviderOrSigner(true);
            // @ts-ignore
            const address: string = await signer.getAddress(); // `getAddress()` -> ethers' func
            ...
            console.error(error);
            // ** Wouldn't `tokensToBeClaimed` be `zero` either way here?
            setTokensToBeClaimed(zero);
        }
    };
    ...
    const mintCryptoDevToken = async (amount: number) => {
        try {
            ...
            const tx = await tokenContract.mint(amount, {
                ...
                // We are parsing `0.001` string to ether (BigNumber) using the utils library from ethers.js
            ...
            // ** Why are we fetching these 3 values?
    ...
    const claimCryptoDevTokens = async () => {
        try {
            ...
            await getTotalTokensMinted();
            await getTokensToBeClaimed();
            // ** Why aren't we running `setTokensToBeClaimed` here?
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const withdrawCoins = async () => {
        try {
            ...
            await tx.wait();
            setLoading(false);
            // ** Shouldn't we validate if we func is called by the owner on the frontend as well (for safe measures?)
    ...
    useEffect(() => {
        ...
        if (!walletConnected) {
            ...
            // I guess we're running all these getters for UI changes
            getTotalTokensMinted();
            getBalanceOfCryptoDevTokens();
            getTokensToBeClaimed();

            // ** Why are we calling `withdrawCoins()` here?
            withdrawCoins();
        }
    }, [walletConnected]);
    ...
    const renderButton = () => {
        ...
        // if owner is connected, withdrawCoins() is displayed
        ...
        if (tokensToBeClaimed.toNumber() > 0) {
            return (
                <div>
                    <div className="description">{tokensToBeClaimed.toNumber() * 10} Tokens can be claimed!</div>
        ...
        <div>
            ...
            <div className="main">
                <div>
                    ...
                    <div className="description">You can claim or mint Crypto Dev tokens here</div>
                    {walletConnected ? (
                        <div>
                            <>
